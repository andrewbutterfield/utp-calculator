...
These reduce functions have an additional predicate argument,
which is provided as a means to supply context-dependent invariants.
The non-compositional UTCP variant described here does not need this
information, but more recent work on a fully compositional variant does.
We consider its use outside the scope of this paper.


A simple example of such a pattern is the following encoding
of $\Skip;P = P$ :
\begin{code}
reduceUTCP d _ -- ignore invariant pred.
 (Comp "Seq" [(Comp "Skip" []), pr]) = lred ";-lunit" pr
\end{code}
The pattern
...
($P$ in the law above).
The righthand-side returns the application \verb'lred ";-lunit" pr'
which simply constructs a \texttt{RWResult} return value,
...
creduceUTCP d (PSub w@(Comp "Iter" [c,p]) sub)
 ...
 where
   csub = PSub c sub
   ctrue  = ( psimp d csub -- psimp runs the simplifier directly
            , mkSeq (PSub p sub) w, diff )
   cfalse = ( psimp d $ mkNot csub
            , PSub mkSkip sub, diff )
\end{code}
...
took the simple command `\texttt{l}' to produce the conditional form,
and a command \verb@"ln"@, with \texttt{n} a digit,
to unroll the disjunctive form $n$ times.
